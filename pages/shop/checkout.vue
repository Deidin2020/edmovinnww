<template>
    <div id="main-wrapper" class="main-wrapper">

        <section class="checkout-page-area section-gap-equal">
            <div class="container">
                <form>
                    <!-- Coupon -->
                    <div class="checkout-notice">
                        <div class="coupn-box">
                            <h6 class="toggle-bar"> Have a coupon?
                                <a class="toggle-btn checkout-page-vue-coupon-form-toggle" data-bs-toggle="collapse"
                                    data-bs-target="#collapseTwo">
                                    Click here to enter your code
                                </a>
                            </h6>
                            <div id="collapseTwo" class="checkout-page-vue-coupon-form collapse">
                                <p>If you have a coupon code, please apply it below.</p>
                                <div class="input-group">
                                    <input placeholder="Enter coupon code" type="text">
                                    <div class="apply-btn">
                                        <button type="submit" class="edu-btn btn-medium">Apply</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="row row--25">
                        <!-- Billing Details -->
                        <div class="col-lg-6">
                            <div class="checkout-billing">
                                <h3 class="title">Billing Details</h3>
                                <div class="row g-lg-5">
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <label>First Name*</label>
                                            <input type="text" id="first-name">
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <label>Last Name*</label>
                                            <input type="text" id="last-name">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label>Company Name</label>
                                    <input type="text" id="company-name">
                                </div>
                                <div class="form-group">
                                    <label>Email Address*</label>
                                    <input type="email" id="email">
                                </div>
                                <div class="row g-lg-5">
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <label>Phone*</label>
                                            <input type="tel" id="phone">
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <label>Country*</label>
                                            <select id="country">
                                                <option value="0">Select Option</option>
                                                <option value="3">Australia</option>
                                                <option value="4">England</option>
                                                <option value="6">New Zealand</option>
                                                <option value="5">Switzerland</option>
                                                <option value="1">United Kingdom (UK)</option>
                                                <option value="2">United States (USA)</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label>Address*</label>
                                    <input type="text" id="address1">
                                    <input type="text" id="address2">
                                </div>
                                <div class="form-group">
                                    <label>Town/ City*</label>
                                    <input type="text" id="town">
                                </div>
                                <div class="row g-lg-5">
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <label>State*</label>
                                            <select id="state">
                                                <option value="0">Select Option</option>
                                                <option value="1">Australia</option>
                                                <option value="2">England</option>
                                                <option value="3">New Zealand</option>
                                                <option value="4">Switzerland</option>
                                                <option value="5">United Kingdom (UK)</option>
                                                <option value="6">United States (USA)</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            <label>Zip Code*</label>
                                            <input type="tel" id="zip">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="edu-form-check">
                                        <input type="checkbox" id="crt-accnt" class="w-25">
                                        <label for="crt-accnt">Create an Account?</label>
                                    </div>
                                </div>
                                <div class="form-group mt--50 mb-0">
                                    <label>Order Notes</label>
                                    <textarea id="notes" rows="4"
                                        placeholder="Notes about your order, e.g. special notes for delivery."></textarea>
                                </div>
                            </div>
                        </div>

                        <!-- Order Summary -->
                        <div class="col-lg-6">
                            <div class="order-summery checkout-summery">
                                <div class="summery-table-wrap">
                                    <h4 class="title">Your Orders</h4>
                                    <table class="table summery-table" v-if="cart.length">
                                        <tbody>
                                            <tr v-for="room in cart" :key="room.id">
                                                <td>{{ room.name }} <span class="quantity">x {{ room.quantity }}</span>
                                                </td>
                                                <td>${{ (cleanPrice(room.price) + cleanPrice(room.deposit)) *
                                                    room.quantity }}</td>
                                            </tr>
                                            <tr class="order-subtotal">
                                                <td>Sub Total</td>
                                                <td>${{ subtotal }}</td>
                                            </tr>
                                            <tr class="order-total">
                                                <td>Order Total</td>
                                                <td>${{ finalTotal }}</td>
                                            </tr>
                                        </tbody>
                                    </table>

                                    <!-- Empty Cart -->
                                    <div v-else class="text-center py-5">
                                        <h4>Your cart is empty</h4>
                                    </div>
                                </div>
                            </div>

                            <!-- Payment Methods -->
                            <div class="order-payment">
                                <h4 class="title">Payment</h4>
                                <div class="payment-method" v-if="cart.length">
                                    <div class="form-group" v-for="method in paymentMethods" :key="method.id">
                                        <div class="edu-form-check">
                                            <input type="radio" :id="method.id" name="payment">
                                            <label :for="method.id">{{ method.label }}</label>
                                            <p v-if="method.description">{{ method.description }}</p>
                                        </div>
                                    </div>
                                </div>
                                <a href="#" class="edu-btn order-place" v-if="cart.length">Place Your order <i
                                        class="icon-4"></i></a>
                            </div>
                        </div>
                    </div>
                </form>
            </div>
        </section>

    </div>
</template>

<script>
export default {
  data() {
    return {
      cart: [],
      paymentMethods: [
        {
          id: 'pay-bank',
          label: 'Direct Bank Transfer',
          description: 'Please send a check to Store Name, Store Street, Store Town, Store State / County, Store Postcode.'
        }
      ]
    };
  },
  computed: {
    subtotal() {
      return this.cart.reduce((sum, r) => sum + this.cleanPrice(r.price) * r.quantity, 0);
    },
    totalDeposit() {
      return this.cart.reduce((sum, r) => sum + this.cleanPrice(r.deposit) * r.quantity, 0);
    },
    finalTotal() {
      return this.subtotal + this.totalDeposit;
    }
  },
  mounted() {
    const stored = JSON.parse(localStorage.getItem("cartRooms")) || [];
    this.cart = stored.map(r => ({ ...r, quantity: r.quantity || 1 }));
  },
  methods: {
    cleanPrice(value) {
      if (!value) return 0;
      return Number(value.toString().replace(/[^0-9.-]+/g, '')) || 0;
    }
  },
  head() {
    return { title: 'Checkout Page' };
  }
};
</script>
